...
ARGUMENTS = {
    ...
    'beer_description': {'help': 'Description of beer', 'type': str, 'default': '????'},
    'beer_class': {'help': 'Type of beer', 'type': str, 'default': 'Bière pur malt.'},
}
...
def create_label(product_name=None, abv=None, volume=None, batch=None, expires=None, allergens=None, beer_description=None, beer_class=None):
    ...
    if not beer_class: beer_class = ARGUMENTS['beer_class']['default']
    ...
    im.paste(pregnant_logo, box=(570, 130), mask=pregnant_logo.split()[3])
    ...
    font = fit_text(beer_description, LABEL_SIZE[0], font_path, ifs)
    draw.text((x_pos, y_pos), beer_description, font=font, fill=0)
    ...
    txt = "// Nous lavons les bouteilles,"
    y_pos += 40
    font_path = resource_path(FONT_REGULAR)
    ifs = int(4*FONT_SCALING) # initial font size
    font = fit_text(txt, LABEL_SIZE[0], font_path, ifs, break_func=break_text_whitespace, max_lines=2)
    ...
    txt = "pensez à nous la rapporter ! //"
    y_pos += 20
    font_path = resource_path(FONT_REGULAR)
    ifs = int(4*FONT_SCALING) # initial font size
    font = fit_text(txt, LABEL_SIZE[0], font_path, ifs, break_func=break_text_whitespace, max_lines=2)
    ...
    font = fit_text(beer_class, LABEL_SIZE[0], font_path, ifs, break_func=break_text_whitespace, max_lines=2)
    line_spacing = int(font.size * 1.15)
    for i, line in enumerate(break_text_whitespace(beer_class, font, LABEL_SIZE[0])):
        draw.text((x_pos, y_pos+line_spacing*i), line, font=font)
    ...
    txt = "Microbrasserie Urbaine Le Singe Savant"
    x_pos += 397
    y_pos -= 15
    font_path = resource_path(FONT_BOLD)
    ifs = int(3*FONT_SCALING) # initial font size
    font = fit_text(txt, LABEL_SIZE[0], font_path, ifs, break_func=break_text_whitespace, max_lines=2)
    line_spacing = int(font.size * 1.15)
    for i, line in enumerate(break_text_whitespace(txt, font, LABEL_SIZE[0])):
        draw.text((x_pos, y_pos+line_spacing*i), line, font=font)

    txt = "http://www.singe-savant.com"
    y_pos += 12
    x_pos += 53
    font_path = resource_path(FONT_BOLD)
    ifs = int(3*FONT_SCALING) # initial font size
    ...
    txt = "134 Rue D'arras - 59000 LILLE"
    y_pos += 12
    x_pos -= 6
    font_path = resource_path(FONT_BOLD)
    ifs = int(3*FONT_SCALING) # initial font size
